...
data = json.loads((out / "browser-audit-v18-live-sync-3.json").read_text(encoding="utf-8"))

console_lines = [
    "BROWSER=Codex In-app Browser",
    "PUBLIC_RELEASE=V18-LIVE-SYNC-3",
    "ROOT_ERRORS=0",
    "PRESENTATION_ERRORS=0",
    "RAPIDE_PAGE_ERRORS=0",
    "GPT_PAGE_ERRORS=0",
    "VISITE_ERRORS_AFTER_START=2",
]
for error in data["pages"]["/visite/"]["pointerLockErrors"]:
    console_lines.append(
        f"VISITE_ERROR level={error['level']} url={error.get('url', '')} message={error['message']}"
    )
(out / "console-errors-sync-3.txt").write_text("\n".join(console_lines) + "\n", encoding="utf-8")

network_lines = ["BROWSER=Codex In-app Browser", "PUBLIC_RELEASE=V18-LIVE-SYNC-3"]
for page in ["/", "/presentation/", "/visite/", "/rapide/", "/gpt/"]:
    resources = data["pages"][page]["cdp"]["resources"]
    failed = [item for item in resources if item.get("responseStatus") not in (None, 200)]
    network_lines.append(
        f"PAGE={page} RESOURCES={len(resources)} "
        f"STATUS_200={sum(1 for item in resources if item.get('responseStatus') == 200)} "
        f"FAILED={len(failed)}"
    )
    for item in resources:
        network_lines.append(
            f"  {item.get('responseStatus')} {item.get('initiatorType')} "
            f"{item.get('decodedBodySize')} {item.get('name')}"
        )
network_lines += [
...
(out / "network-status-sync-3.txt").write_text("\n".join(network_lines) + "\n", encoding="utf-8")

outside = data["pages"]["/visite/"]["cameraTrace"]["outside"]
garden = data["pages"]["/visite/"]["cameraTrace"]["garden"]

report = f'''# Réaudit navigateur public — Chamagnieu V18-LIVE-SYNC-3

- Date UTC : `{data['auditedAt']}`
...
(out / "BROWSER_REAUDIT_V18_SYNC_3.md").write_text(report, encoding="utf-8")

for path in [
    out / "BROWSER_REAUDIT_V18_SYNC_3.md",
    out / "browser-audit-v18-live-sync-3.json",
    out / "console-errors-sync-3.txt",
    out / "network-status-sync-3.txt",
]:
    payload = path.read_bytes()
    print(f"{path.name} bytes={len(payload)} sha256={hashlib.sha256(payload).hexdigest().upper()}")
